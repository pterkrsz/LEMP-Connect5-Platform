...
{
    private const string InfluxWriteUrl = "http://localhost:8181/api/v3/write_lp?db=local_system&precision=nanosecond";
    ...
    {
        var user = context.User?.Identity?.IsAuthenticated == true ? context.User.Identity!.Name ?? "anonymous" : "anonymous";
        var role = context.User?.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Role)?.Value ?? "none";
        var method = context.Request.Method;
        var path = context.Request.Path.HasValue ? context.Request.Path.Value! : string.Empty;
        var status = context.Response.StatusCode;
        var ip = context.Connection.RemoteIpAddress?.ToString() ?? "unknown";
        var id = Guid.NewGuid().ToString("N");
        var timestamp = (DateTime.UtcNow - DateTime.UnixEpoch).Ticks * 100;
        var line = BuildLineProtocol(id, user, role, method, path, status, durationMs, ip, timestamp);

        var client = _httpClientFactory.CreateClient("Influx");
    ...
    private static string EscapeField(string value) => value.Replace("\\", "\\\\").Replace("\"", "\\\"");
}